...
{
    public class SubtractMatricesMethod : IMatrixMethod
    {
        public string Name => "Subtract Matrices";

        public string Execute(MatrixModel A)
        {
            throw new InvalidOperationException("Subtraction requires two matrices.");
        }

        public string Execute(MatrixModel A, MatrixModel B)
        {
            if (A.Rows != B.Rows || A.Columns != B.Columns)
                return "Subtraction requires matrices of same size.";
